...
function Registration() {
    ...
    const checkEmptyFields = () => {
        return user.email !== '' || user.firstName !== '' || user.lastName !== '' || user.email !== '' || user.password !== '' || user.passwordConf !== '' ? true : false
    }

    const handleRegistration = () => {
        setRegMessage('')
        if (!checkEmptyFields())
            setRegMessage('Fill all fields')
        let text = user.password===user.passwordConf?'':'Check your confirmation password'
        setRegMessage(text)
                        ...
                        <Card.Body>
                            ...
                            <Form>
                                ...
                                <Form.Group>
                                    ...
                                    <Form.Control type='password' id="password" value={user.passwordConf} onChange={e=>{setUser({...user, passwordConf: e.target.value})}}/>
                                </Form.Group>
                            ...
                            <Alert style={{marginTop:'15px'}} variant='danger' className={regMessage!=='' ? '' : 'd-none'}>{regMessage}</Alert>
                        </Card.Body>
                    </Card>
                </Col>
            </Container>
